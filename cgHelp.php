...
<li><a href="#StoredData">Using Stored Templates and Recipient Lists for Campaign Generation</a>
<li><a href="#validate">Validation Process to make sure your data matching your template</a></li>
...
<tbody>
<tr>
<td colspan="2"><strong>Welcome</strong> to Campaign Generator. First things first.... This tool was not created nor is it supported by SparkPost. This tool was created by some SparkPost community contributors and is open for anyone to use. This tool is free to use at your own risk and should be up and running at all times but is not monitored for uptime. The Campaign Generator DOES NOT hold any of your information, it simply uses your API key to obtain enough information in order to create, schedule or cancel your campaigns. The code behind this site is available from my Github repository at: https://github.com/jeff-goldstein/sparkpost/tree/master/cgCampaignSubmission.</td>
</tr>
<tr>
<td colspan="2"><strong>I</strong> created this campaign&nbsp;generation tool to fill in missing functionality from SparkPost. Since SparkPost is geared toward developers, anyone who needs to send Campaigns either had to build the functionality themselves or use one of SparkPost's partners. So I created this tool as a quick way to either launch a campaign now or to schedule it out for a later release. When using this tool, please understand the nuances of SparkPost because I won't document them in this tool. For example as of Sept 2016, campaigns can only be cancelled if they are scheduled for a time more than one day out. I'm not going to document all of these nuances and will let SparkPost documentation speak for itself; so if something isn't exactly working the way you expected it to work, please check their documentation first at, <a href="https://developers.sparkpost.com/">SparkPost Documentation</a></td>
</tr>
<tr>
...
<p><strong>At a minimum, the API Key needs the following authentication rights:<i><br><br>'Recipient Lists: Read/Write, Templates: Read/Write, Transmissions: Read/Write' and Sending Domains: Read.</strong></i></p>
...
<p><strong>Here</strong> is one of two ways to submit a campaign. This approach uses the API Key to call the SparkPost server to compile a list of your published Templates and Recipient Lists that have been uploaded to the system. The first step is to select which Templates and Recipients you are interested in using for this campaign.</p>
<a name="validate"></a>
<h4>Validation</h4>
<p>If you want to check to see that you have a good match, press the 'Preview & Validate' button directly below the Recipient list. At this time, the system will do several things:</p>
<ol>
<li>The Campaign Generator will grab the substitution data from the first recipient in the selected recipient list.</li>
<br>
<li>Display what the email will look like by merging the recipient data with the template. The preview will be displayed at the bottom of the page in the Preview section.  Please look at the content closely to see if it looks like the data is filling out the template correctly.</li>
<br>
<li>The Campaign Generator will compare how the data and the template matches up in two different ways (see the lower table to the right of the Campaign Submission Form):<br />
<br><ul>
...
<p>Once you submit the data, the campaign will be scheduled and you will be taken to the Receipts page.</p>
</td>
<td valign="top"><a href="https://dl.dropboxusercontent.com/u/4387255/cgScreenSnapshots/cgBuildSubmitStored.png" target="_blank"><img src="https://dl.dropboxusercontent.com/u/4387255/cgScreenSnapshots/cgBuildSubmitStored.png" alt="Campaign Generator Login Page" width="800" height="800&quot;" /></a></td>
</tr>
<tr>
<td valign="top">
...
<tbody>
<tr>
<td>{"recipients":[{"address":"jeff.goldstein@sparkpost.com","UserName":"Sam Smith","substitution_data":{"first":"Sam","id":"342","city":"Princeton"}}, {"address":"austein@hotmail.com","UserName":"Fred Frankly","substitution_data":{"first":"Fred","id":"38232","city":"Nowhere"}}, {"address":"jeff@geekswithapersonality.com","UserName":"Zachary Zu</td>
</tr>
</tbody>
</table>
<p>Yes that is ugly; so by using a json formatter (<a href="http://jsonlint.com/#">JSONLINT</a>), you can see what the output looks like in human readable form. Notice that the dash does not show up in your data:</p>
<pre>{
 "recipients": [{
...
<p>The only other difference between this process and using stored information is that you may need to enter a 'global return path'. This is only needed for Enterprise/Elite users. The system will automatically get a list of validated domains that are available for your API Key. Enter in the name you want to use and the system will combine the two into a proper email address.</p>
<p>A subtle difference between this Campaign Generation process and the one that uses stored recipients is that we don't have a 'Send Test Email' button on this page. The reason for that is because you can enter in a set of users to get test emails by placing them in the Recipient data field. By taking this approach, you can send different data sets to different people so you can really see how your template is performing. Unlike the stored recipient approach that only uses the data from the first recipient, this will send to everyone in the recipient data field. So you if you want 10 people to get a test email; have 10 recipients along with their individual data and submit the campaign just like you would the real campaign.</p>
</td>
<td valign="top"><a href="https://dl.dropboxusercontent.com/u/4387255/cgScreenSnapshots/cgBuildSubmitCSVJSON.png" target="_blank"><img src="https://dl.dropboxusercontent.com/u/4387255/cgScreenSnapshots/cgBuildSubmitCSVJSON.png" alt="Campaign Generator Login Page" width="800" height="800&quot;" /></a></td>
</tr>
<tr>
<td valign="top">
<h4><a id="Commit" name="Commit"></a>Commit Page</h4>
...
<p><code>{ "results": { "total_rejected_recipients": 0, "total_accepted_recipients": 1, "id": "102445114446875802" } }</code>.</p>
<p>If not, you will see some server errors that might look like:</p>
<p><code>{ "errors": [ { "message": "Failed to generate message", "description": "[internal] Error while rendering part html: line 64: substitution value 'dynamic_html' did not exist or was null", "code": "1901" } ], "results": { "total_rejected_recipients": 0, "total_accepted_recipients": 1, "id": "66416320824814490" } } </code></p>
<p>As a precaution, the Schedule Campaign Button will change text and color to reflect that the campaign was submitted to the server and it either worked or didn't. So again, check the results closely. If you entered in an email address in the Generation page, an email should be sent out immediately.</p>
</td>
<td valign="top"><a href="https://dl.dropboxusercontent.com/u/4387255/cgScreenSnapshots/cgConfirmSubmission.png" target="_blank"><img src="https://dl.dropboxusercontent.com/u/4387255/cgScreenSnapshots/cgConfirmSubmission.png" alt="Campaign Generator Login Page" width="800" height="800&quot;" /></a></td>
</tr>
<tr>
<td valign="top">
<h4><a id="Scheduled" name="Scheduled"></a>Scheduled Campaigns</h4>
